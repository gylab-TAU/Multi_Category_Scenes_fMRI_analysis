...
roi_list = c("Place_dist", "Body_dist", "Body_Place") # for LOC
...
curr_betas = filter(curr_betas, subj!="15" )
...
curr_roi = filter(curr_betas_wide, roi=="Body_Place")
diff_vec = curr_roi$b_Room-curr_roi$b_Body
t.test(diff_vec)
mean(diff_vec)/sd(diff_vec)

curr_roi = filter(curr_betas_wide, roi=="Body_Place")
diff_vec = (curr_roi$b_Room+curr_roi$b_Body)/2-(curr_roi$b_Chair+curr_roi$b_Face)/2
t.test(diff_vec)
mean(diff_vec)/sd(diff_vec)

curr_roi = filter(curr_betas_wide, roi=="Body_Place")
diff_vec = (curr_roi$b_Room+curr_roi$b_Body)/2-curr_roi$b_Chair
t.test(diff_vec)
mean(diff_vec)/sd(diff_vec)

curr_roi = filter(curr_betas_wide, roi=="Body_Place")
diff_vec = (curr_roi$b_Room+curr_roi$b_Body)/2-curr_roi$b_Face
t.test(diff_vec)
mean(diff_vec)/sd(diff_vec)
