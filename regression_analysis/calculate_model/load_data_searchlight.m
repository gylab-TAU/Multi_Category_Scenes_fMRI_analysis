...
data.subj_name = settings.subj_file;
num_voxels = size(subj_data.data,1);
data.spm_vol = subj_data.spm_vol;

data.single_cond_names = settings.single_cond_names;
data.combined_cond_name = settings.combined_cond_name;
...
%% filter dist contrasts

data.dist_t_vals = zeros(size(data.orig_data,1),length(settings.dist_contrasts_names));
data.dist_t_names = settings.dist_contrasts_names;
                           
data.dist_contrast_th = settings.dist_contrast_th;

for cont_itr=1:length(data.dist_t_names)
    
    filter_t_val_header = data.dist_t_names{cont_itr};
    filter_t_val_ind = find(~cellfun(@isempty,regexp(subj_data.header, ['^' filter_t_val_header '$'])));

    data.dist_t_vals(:,cont_itr) = subj_data.data(:,filter_t_val_ind);
%     data.loc_t_vals(:,cont_itr) = data.loc_t_vals(:,cont_itr)(data.conj_mask,:);
end
