...
%             if(strmatch('0',d(runs_id).name))

                %for SPGR
    %             temp_dir = dir([this_subject_root_spm_path,filesep,d(runs_id).name]);
    %             temp_dir = dir([this_subject_root_spm_path,filesep,session_id_str]);
    %             if (length(temp_dir)<10)
    %                 continue;
    %             end    
